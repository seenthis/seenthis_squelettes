...
function formulaires_profil_traiter() {
	...
	if ($email != $email_ancien) {
		...
		foreach (
			...
			$msg = _T('seenthis:changement_email_informer', [
				'login' => '@' . $GLOBALS['auteur_session']['login'],
				'new' => $email,
				'old' => $email_ancien
			]);
			$envoyer_mail = charger_fonction('envoyer_mail', 'inc');
			$titre_mail = _T('seenthis:changement_email_subject', [
				'login' => '@' . $GLOBALS['auteur_session']['login']
			]);
	...
	if (strlen($nom_fond) > 0) {
		if (!preg_match(',\.(jpe?g|png|gif)$,i', $nom_fond)) {
			$errors['image_fond'] = 'Mauvais format.';
		} else {
	...
	if (strlen($nom_logo) > 0) {
		if (!preg_match(',\.(jpe?g|png|gif)$,i', $nom_logo)) {
			$errors['image_logo'] = 'Mauvais format.';
		} else {
			...
			if ($f = fichier_logo_auteur($id_auteur)) {
				@unlink($f);
			}
