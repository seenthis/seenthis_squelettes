$(function () {
	 ...
	 */
	function createImage(parentDiv, imageUrl, lienId, elementsLiens, afficheImage, masqueLiens, afficheMessageLienDejaPoste) {
		var tmpImg = $('<img>')
			.on('load', function () {
	...
	function suivreEdition() {
		$('textarea:not(.suiviEdition)').each(function (_, areaDom) {
			var area = $(areaDom);
			area.addClass('suiviEdition');
			...
			var mesTraductions;
			if (typeof traductionSeenThis === 'undefined') {
				mesTraductions = {}
			} else {
				mesTraductions = traductionSeenThis;
			}

			function raccourcisDo(formulaire, before, after) {
				var textArea = formulaire.find('textarea');
				textArea.replaceSelection(before + jQuery.trim(textArea.getSelection().text) + after, true);
			}
			...
			if (currentForm.find('.people').length == 0) {
				$('<div class="people"></div><div style="clear: left;"></div><div class="tags"></div><div style="clear: left;"></div><div class="liens"></div><div style="clear: left;"></div><div class="images"></div><div style="clear: left;"></div>').appendTo(currentForm);
			}
			...
			if (currentForm.find('.raccourcis').length == 0) {
				var raccourcis = $('<div class="raccourcis"></div>').prependTo(currentForm);

				var raccourcisQuote = $('<a class="raccourci_quote" href="#">' + mesTraductions.raccourci_quote + '</a>').appendTo(raccourcis);
				raccourcisQuote.click(function () {
					raccourcisDo(currentForm, '\n❝', '❞\n');
					return false;
				});
				var raccourcisItalic = $('<a class="raccourci_italic" href="#">' + mesTraductions.raccourci_italic + '</a>').appendTo(raccourcis);
				raccourcisItalic.click(function () {
					raccourcisDo(currentForm, ' _', '_ ');
					return false;
				});
				var raccourcisBold = $('<a class="raccourci_bold" href="#">' + mesTraductions.raccourci_bold + '</a>').appendTo(raccourcis);
				raccourcisBold.click(function () {
					raccourcisDo(currentForm, ' *', '* ');
					return false;
				});
			}
